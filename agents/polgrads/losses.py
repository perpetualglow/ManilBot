...
class BatchQPGLoss(object):
  ...
  def loss(self, policy_logits, action_values, legal_actions):
    ...
    """
    pol =  tf.add(policy_logits, legal_actions)
    _assert_rank_and_shape_compatibility([pol, action_values], 2)
    advantages = compute_advantages(pol, action_values)
...
class BatchRMLoss(object):
  ...
  def loss(self, policy_logits, action_values, legal_actions):
    ...
    pol = tf.add(policy_logits, legal_actions)
    _assert_rank_and_shape_compatibility([pol, action_values], 2)
    advantages = compute_advantages(pol, action_values, use_relu=True)
    _assert_rank_and_shape_compatibility([advantages], 1)
    total_adv = tf.reduce_mean(advantages, axis=0)
