...
import agents.polgrads.network as simple_nets
import agents.polgrads.losses as rl_losses
...
class PolicyGradient():
  ...
  def __init__(self,
    ...
    self._legal_ph = tf.placeholder(
        shape=[None, num_actions], dtype=tf.float32,  name="legal_ph")
    ...
    if loss_class.__name__ == "BatchA2CLoss":
      ...
      self._pi_loss = pg_class.loss(
          policy_logits=self._policy_logits, action_values=self._q_values, legal_actions=self._legal_ph)
    if optimizer_str == "adam":
  ...
  def _add_episode_data_to_dataset(self):
    ...
    legal_actions = [data.legal_actions_mask for data in self._episode_data]
    ...
    self._dataset["legal_actions_mask"].extend(legal_actions)
    self._episode_data = []
  ...
  def _add_transition(self, time_step):
    ...
    legal_actions_mask = np.ones(self._num_actions) * -1e32
    legal_actions_mask[legal_actions] = 0
  ...
  def _critic_update(self):
    ...
    critic_loss, _ = self._session.run(
        ...
        feed_dict={
            ...
            self._legal_ph: self._dataset["legal_actions_mask"]
        })
