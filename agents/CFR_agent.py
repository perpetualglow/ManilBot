...
import collections
import attr
from util.openspiel_policy import get_all_states, TabularPolicy
...
class _CFRSolverBase(object):
    ...
    def __init__(self, game, states, alternating_updates, linear_averaging,
        ...
        self._current_policy = TabularPolicy(game, states)
    ...
    def _initialize_info_state_nodes(self, state):
        ...
        current_player = state.get_current_player_id()
    ...
    def action_probs(self, obs, encoded_legal_actions, legal_actions):
        return self.average_policy().action_probs(obs, encoded_legal_actions, legal_actions)

    def action_probabilities(self, state):
        return self.average_policy().action_probabilities(state)
    ...
    def _compute_counterfactual_regret_for_player(self, state, policies,
        ...
        """
        if state.is_over():
            return np.asarray(state.get_payoffs())
...
class _CFRSolver(_CFRSolverBase):
    ...
    def evaluate_and_update_policy(self, state):
